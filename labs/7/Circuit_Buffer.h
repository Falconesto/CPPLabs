...
public:
    ...
    inline T& operator[](const int& rhs) {return ptr_[rhs];}
...
public:
    ...
    circular_buffer(unsigned capacity = 100) : capacity_(capacity)
    {
        if (capacity == 0){
    ...
    circular_buffer(const circular_buffer<T>& other): capacity_(other.capacity_)
    {
        start_ = new buffer_element<T>();
    ...
    void push_mid(T value, int position) {
        if(position>size()){
